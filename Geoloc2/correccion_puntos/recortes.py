    ...
    for cve in tab_f['CVEGEO_MANZANA'].unique():
        ...
        if ((float(df['diff']) > 0) & (int(df['Conteo_final'])!=0)): 
                ...
                while (int(tab2['diff'].unique())>0) is True :
                    ...
                    pdf=geom_clas8.loc[(geom_clas8['CVEGEO_MANZANA']==cve)].reset_index(drop=True).iloc[:int(tab2['diff'])]
                #     pdf.drop(columns=['CLAVECATASTRAL', 'BDINTERNA_CONT_NOMBRE_COMPLETO', 'BDINTERNA_CONT_RFC',
                # 'BDINTERNA_CONT_EMAIL', 'BDINTERNA_CONT_DIRECCION'],inplace=True)
                    pdf.drop(columns=['CLAVECATASTRAL_RECAUDACION'],inplace=True)
                        
                    # print(geom_clas8.loc[geom_clas8['CVEGEO_MANZANA']==cve])
                    zdf=geom_clas8.loc[geom_clas8['CVEGEO_MANZANA']==cve]
                    ndf=pd.concat([pdf, zdf, ndf], axis=0)
                
                    pdf2= pd.concat([ndf,pdf2],axis=0)
                    tab2= pd.merge(df,  ndf.groupby('CVEGEO_MANZANA').count()['Latitud'].reset_index(), on ='CVEGEO_MANZANA')
                    # print(tab2)
